...
then
  BUILD_TYPE=RELEASE
  BUILD_SUFFIX=opt
  BUILD_C_FLAGS="-mkl -qoverride-limits"
  BUILD_CXX_FLAGS="-mkl -qoverride-limits"
  BUILD_F_FLAGS="-mkl -qoverride-limits"
...
if [[ "${USE_MPI:?}" == "mpi" ]]
then
    TRIL_INSTALL_DIR=${SPARC_ARCH}_${SPARC_COMPILER}_${SPARC_MPI}_${EXECUTIONSPACE:?}_${LINKTYPE:?}_${VARIANT:?}
else
